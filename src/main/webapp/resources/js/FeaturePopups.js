...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
   ...
   initialize : function(options) {
      ...
      if (options.selectOptions !== null) {
         ...
         var selectClass = OpenLayers.Class(OpenLayers.Control.SelectFeature, {
            ...
            highlight : function(feature) {
               ...
               if (controlHover && _lastHighlighter
                  ...
                  controlHover.events.triggerEvent('featureunhighlighted', {
                     feature : feature
                  });
               }
            }
         ...
         if (this.boxSelectionOptions) {
            ...
            this.handlerBox = new OpenLayers.Handler.Box(this, {
         ...
         control.unselectAll = function(options) {
            ...
            OpenLayers.Control.SelectFeature.prototype.unselectAll.apply(this, arguments);
            self.unselectingAll = false;

            var exceptLayerId, layerObjs = self.layerObjs;
            if (options && options.except) {
               exceptLayerId = options.except.layer.id;
            }
            var layers = this.layers || [ this.layer ];
            for ( var i = 0, len = layers.length; i < len; i++) {
               var layerId = layers[i].id, layerObj = layerObjs[layerId];
               if (layerObj.safeSelection) {
                  ...
                  if (layerId === exceptLayerId) {
                     ...
                     var selLayerFeature = layerObj.layer.selectedFeatures;
                     if (selLayerFeature.length && selLayerFeature[0].id === options.except.id) {
                        layerObj.refreshSelection();
                     }
                  }
               } else {
      ...
      // ---------------------
      var _closeBox = !!(this.mode & MODES.CLOSE_BOX), _unselectFunction = (this.mode & MODES.UNSELECT_ON_CLOSE
            ? function() {
      ...
      this.popupObjs = OpenLayers.Control.FeaturePopups_Utils.createPopupObjs(this, popupOptions, defaultPopupOptions);
    ...
    */
   setMap : function(map) {
      if (this.boxSelectionOptions && this.boxSelectionOptions.keyMask === OpenLayers.Handler.MOD_CTRL) {
   ...
   addLayers : function(layers) {
      var added = false, response, layerItem;
      for ( var i = 0, len = layers.length; i < len; i++) {
         layerItem = layers[i];
   ...
   addLayerToControl : function(layer, options) {
      var layerObj = this.getLayerObj(layer), response = false;
      if (!layerObj) {
         options = OpenLayers.Util.applyDefaults(options, {
            templates : {}
         });
         ...
         OpenLayers.Util.applyDefaults(options.templates, {
            list : (oTemplates.item
                  ? this.layerListTemplate
                  : ''),
            hoverList : ((oTemplates.hover || oTemplates.hoverItem)
                  ? this.hoverClusterTemplate
                  : '')
         });
         layerObj = new OpenLayers.Control.FeaturePopups.Layer(this, layer, options);
   ...
   onSelectBox : function(position) {
      ...
      OpenLayers.Control.SelectFeature.prototype.selectBox.apply(this.controls.select, arguments);
   ...
   unselectLayer : function(layer) {
      ...
      if (selectedFeatures) {
         var selControl = this.controls.select, layerObj = this.getLayerObj(layer);
         if (selControl && layerObj
               && this.active) {
            layerObj.unselectLayer(selControl);
            this.refreshLayers();
         }
   ...
   refreshLayers : function(useCursorLocation) {
      var layers = OpenLayers.Array.filter(this.layers, function(layer) {
         return !!layer.map;
      });
      var bounds = new OpenLayers.Bounds(), invalid = false, staticInvalid = false, html = [], selectedFeatures = [];
      ...
      for ( var layerId in this.layerObjs) {
         ...
         if (layerObj.active) {
            ...
            if (layerPopObjs) {
               if (r.invalid) {
                  OpenLayers.Control.FeaturePopups_Utils.showListPopup(layerPopObjs, [ {
                     layerObj : layerObj,
                     layer : layerObj.layer,
                     features : r.features
                  } ], r.bounds, [ r.html ], r.staticInvalid, useCursorLocation, this.controls.select.handlers.feature);
               }
            } else {
      ...
      if (invalid) {
         OpenLayers.Control.FeaturePopups_Utils.showListPopup(this.popupObjs, selectedFeatures, bounds, html, staticInvalid, useCursorLocation,
...
OpenLayers.Control.FeaturePopups.DEFAULT = OpenLayers.Control.FeaturePopups.SAFE_SELECTION | OpenLayers.Control.FeaturePopups.CLOSE_ON_UNSELECT
 ...
 */
OpenLayers.Control.FeaturePopups_Utils.createPopupObjs = function(environments, popupOptions, popupDefaults) {
   var popupManager = OpenLayers.Control.FeaturePopups.Popup, applyDefaults = OpenLayers.Util.applyDefaults;
   var popupObjs = {};
   for ( var key in popupDefaults) {
      var pOptions = popupOptions[key];
      if (pOptions !== null) {
         popupObjs[key] = new popupManager(environments, key, applyDefaults(pOptions, popupDefaults[key]));
      }
   }
   return popupObjs;
};
 ...
 *
 */
OpenLayers.Control.FeaturePopups_Utils.showListPopup = function(popupObjs, selectedFeatures, bounds, html, staticInvalid, useCursorLocation, featureHandler) {
   var feature, lonLat, response = false, listPopupObj = popupObjs.list, singlePopupObj = popupObjs.single;
   ...
   if (singlePopupObj && selectedFeatures.length === 1
      ...
      var selObject = selectedFeatures[0], feature = selObject.features[0], layerObj = selObject.layerObj;
      var rr = layerObj.getSingleHtml(feature);
      if (rr.hasTemplate) {
         if (useCursorLocation && feature.geometry.getVertices().length > 1) {
            lonLat = OpenLayers.Control.FeaturePopups_Utils.getLocationFromHandler(featureHandler, feature);
         } else {
   ...
   if (listPopupObj && !response) {
      listPopupObj.showPopup(selectedFeatures, bounds.getCenterLonLat(), (selectedFeatures.length
            ? html.join('\n')
            : ''), staticInvalid);
   }
};
...
OpenLayers.Control.FeaturePopups_Utils.getLocationFromHandler = function(featureHandler, feature) {
   ...
   var xy = (featureHandler.feature === feature)
         ? featureHandler.evt.xy
         : null;
   return (xy
         ? featureHandler.map.getLonLatFromPixel(xy)
...
OpenLayers.Control.FeaturePopups.Popup = OpenLayers.Class({
   ...
   initialize : function(environments, popupType, options) {
      ...
      // close box
      if (this.closeBox) {
         this.onCloseBoxMethod = OpenLayers.Function.bind(function(evt) {
            this.unselectFunction && this.unselectFunction();
            this.clear();
            OpenLayers.Event.stop(evt);
            this.events.triggerEvent('closedbybox', {
               popupType : this.type
            });
         }, this);
      }
      ...
      if (this.relatedToClear.length === 0 || !OpenLayers.Util.isArray(this.relatedToClear[0])) {
         this.relatedToClear = [ this.relatedToClear ];
      }
      ...
      if (popupClass) {
         ...
         pClass.contentDisplayClass
               ...
               && pClass.map === null) {
            this.popupType = 'OL';
            var pClass = popupClass.prototype, maxSize = this.maxSize, minSize = this.minSize;
            if (maxSize) {
               maxSize = new OpenLayers.Size(isNaN(maxSize.w)
                     ? 999999
                     : maxSize.w, isNaN(maxSize.h)
                     ? 999999
                     : maxSize.h);
            }
            if (minSize) {
               minSize = new OpenLayers.Size(isNaN(minSize.w)
                     ? 0
                     : minSize.w, isNaN(minSize.h)
                     ? 0
                     : minSize.h);
            }
            if (pClass.maxSize) {
            ...
            this.popupClass = OpenLayers.Class(popupClass, {
               ...
               panIntoView : function() {
                  self.panMapIfOutOfView && OpenLayers.Popup.prototype.panIntoView.call(this);
               },
               contentDisplayClass : pClass.contentDisplayClass + ' '
                     + this.control.displayClass
                     + '_'
                     + this.type
            });
         ...
         this.moveListener = {
            ...
            mousemove : function(evt) {
               ...
               if (popup && popup.moveTo) {
                  ...
                  popup.moveTo(map.getLayerPxFromLonLat(map.getLonLatFromPixel(evt.xy)));
               }
            }
         };
      }
   ...
   clear : function() {
      this.clearPopup();
      var iiLen = Math.min(this.relatedToClear.length, this.environments.length);
      for ( var ii = 0; ii < iiLen; ii++) {
         var popupObjs = this.environments[ii].popupObjs, relatedToClear = this.relatedToClear[ii];
         for ( var i = 0, len = relatedToClear.length; i < len; i++) {
            var related = popupObjs[relatedToClear[i]];
            if (related && (related.origin === null || related.origin === this)) {
               related.clearPopup();
            }
         }
      }
   ...
   observeShowPopup : function(div) {
      for ( var i = 0, len = div.childNodes.length; i < len; i++) {
         ...
         if (child.id && OpenLayers.String.startsWith(child.id, 'showPopup-OpenLayers')) {
            OpenLayers.Event.observe(child, 'touchend', OpenLayers.Function.bindAsEventListener(this.showListItem, this));
            OpenLayers.Event.observe(child, 'click', OpenLayers.Function.bindAsEventListener(this.showListItem, this));
         } else {
   ...
   create : function(lonLat, html, panMap) {
      var div, popup, control = this.control;
      ...
      case 'div':
         ...
         break;
      case 'OL':
         var _relatedPopup = null, _relatedAxis = null;
         if (this.relatedSimultaneous) {
            var relatedObj = control.popupObjs[this.relatedSimultaneous.related];
            if (relatedObj && relatedObj.popup
                  && relatedObj.popupType === 'OL') {
               _relatedPopup = relatedObj.popup;
               _relatedAxis = this.relatedSimultaneous.axis;
            }
         }
         popup = new this.popupClass(control.id + '_'
               + this.type, lonLat, new OpenLayers.Size(100, 100), html);
         if (this.anchor) {
         ...
         if (_relatedPopup) {
            var _prevCalcRelativePosition = popup.calculateRelativePosition, _relpopRelPosition = (_relatedPopup.relativePosition || 'tr');
         ...
         break;
      case 'custom':
         var returnObj = this.popupClass(control.map, lonLat, html, this.onCloseBoxMethod, this);
         if (returnObj.div) {
 ...
 */
OpenLayers.Control.FeaturePopups.Layer = OpenLayers
      .Class({
         /**
          * APIProperty: events
          * {<OpenLayers.Events>} Events instance for listeners and triggering
         ...
         initialize : function(control, layer, options) {
            ...
            for ( var templateName in oTemplates) {
               _templates[templateName] = this.prepareTemplate(oTemplates[templateName]);
            }
            ...
            if (this.eventListeners) {
               ...
               this.listenFeatures = !!(this.eventListeners && this.eventListeners['featureschanged']);
            }

            // Layer listeners
            // ---------------
            var mode = control.mode, MODES = OpenLayers.Control.FeaturePopups;
            this.safeSelection = !!(mode & MODES.SAFE_SELECTION);
            this.layerListeners = {
               scope : this,
               'featureselected' : this.onFeatureselected
            };
            if (mode & MODES.CLOSE_ON_UNSELECT
                  || this.safeSelection) {
            ...
            if (options.popupOptions) {
               var _closeBox = !!(mode & MODES.CLOSE_BOX), _unselectFunction = (mode & MODES.UNSELECT_ON_CLOSE
                     ? function() {
               ...
               this.popupObjs = OpenLayers.Control.FeaturePopups_Utils.createPopupObjs([ control, this ], options.popupOptions, defaultPopupOptions);
            }

            // Contexts as a private vars
            var _featureContext = this.featureContext || {}, _listContext = this.listContext || {};
            ...
            var renderTemplate = function(template, context, extendedContext) {
               if (typeof template == 'string') {
                  ...
                  return template.replace(OpenLayers.String.tokenRegEx, _replacer);
            ...
            this.applyTemplate = {
               single : function(feature) {
                  return renderTemplate(_templates.single, feature, _featureContext);
               },
               item : function(feature) {
                  return renderTemplate(_templates.item, feature, _featureContext);
               },
               hover : function(feature) {
                  return renderTemplate(_templates.hover, feature, _featureContext);
               },
               hoverItem : function(feature) {
                  return renderTemplate(_templates.hoverItem, feature, _featureContext);
               },
               list : function(listObj) {
                  return renderTemplate(_templates.list, listObj, _listContext);
               },
               hoverList : function(listObj) {
                  return renderTemplate(_templates.hoverList, listObj, _listContext);
               }
            };
         ...
         prepareTemplate : function(template) {
            if (typeof template == 'string') {
               var _subId = 0, _layerId = this.layer.id;
               template = template.replace(this.regExpShow, function(a) {
                  _subId++;
               ...
               template = template.replace(this.regExpAttributes, '${attributes.');
               return template.replace( // internationalize template.
               this.regExpI18n, function(a, key) {
         ...
         highlightFeature : function(feature) {
            var control = this.control, popupObjHover = control.popupObjs.hover;
            if (!popupObjHover) {
               return;
            }

            popupObjHover.clear();
            var templates = this.templates, template = templates.hover, oContextFeature = this.featureContext;
            if (template) {
               var lonLat = OpenLayers.Control.FeaturePopups_Utils.getLocationFromHandler(control.controls.hover.handlers.feature, feature);
               if (feature.cluster) {
                  if (feature.cluster.length == 1) {
                     ...
                     popupObjHover.showPopup({
                        ...
                        layer : this.layer,
                        feature : feature
                     }, lonLat, this.renderTemplate(template, feature.cluster[0], oContextFeature));
                  } else {
                     var html = '', popupObjHoverList = control.popupObjs.hoverList;
                     if (popupObjHoverList) {
                        var cFeatures = feature.cluster, itemTemplate = templates.hoverItem;
                        if (itemTemplate) {
                           var htmlAux = [];
                           for ( var i = 0, len = cFeatures.length; i < len; i++) {
                              htmlAux.push(this.renderTemplate(itemTemplate, cFeatures[i], oContextFeature));
                           }
                           html = htmlAux.join('\n');
                        }
                        popupObjHoverList.showPopup({
                           ...
                           layer : this.layer,
                           features : cFeatures
                        }, lonLat, this.renderTemplate(templates.hoverList, {
                           layer : feature.layer,
                  ...
                  popupObjHover.showPopup({
                     ...
                     layer : this.layer,
                     feature : feature
                  }, lonLat, this.renderTemplate(template, feature, oContextFeature));
               }
            }
         },
         ...
         onFeatureunselected : function(evt) {
            ...
            if (!control.unselectingAll) {
               if (this.safeSelection) {
                  var savedSF = this.selection, feature = evt.feature;
                  if (savedSF) {
                     if (feature.cluster) {
                        for ( var i = 0, len = feature.cluster.length; i < len; i++) {
                           delete savedSF[this.getFeatureId(feature.cluster[i])];
                        }
                     } else {
         ...
         onBeforefeaturesremoved : function(evt) {
            if (evt.features.length && this.layer.getVisibility()
               ...
               this.delayedRefresh = window.setTimeout(OpenLayers.Function.bind(function() {
                  if (this.layer.getVisibility()) {
                     this.delayedRefresh = null;
                     this.refreshFeatures();
                     this.control.refreshLayers();
                  }
               }, this), this.refreshDelay);
            }
         },
         ...
         onFeaturesadded : function(evt) {
            ...
            var layerId = this.layer.id, control = this.control, features = evt.features, savedSF = this.selection;
            if (!this.isEmptyObject(savedSF)) {
               ...
               var _handlerFeature = selectCtl.handlers.feature, _replaceLastFeature = false;
               if (_handlerFeature.lastFeature && !_handlerFeature.lastFeature.layer) {
         ...
         unselectLayer : function(selControl) {
            var layer = this.layer, selectedFeatures = layer.selectedFeatures, control = this.control;
         ...
         refreshFeatures : function() {
            if (this.listenFeatures) {
               var featuresHash, layer = this.layer, layerFeatures = this.getSingleFeatures(layer.features);
               // get hash
         ...
         refreshSelection : function() {
            ...
            var html = '', features = [], bounds = new OpenLayers.Bounds(), invalid = false, staticInvalid = false;
            if (layer.getVisibility() && layer.inRange) {
               var i, len, feature, selectionHash = [], staticSelectionHash = '', layerSelection = this.layer.selectedFeatures, oContextFeature = this.featureContext;
               if (this.safeSelection) {
                  var savedSF = this.selection;
               ...
               var layerTemplate = this.templates.list, htmlAux = [], itemTemplate = this.templates.item;
               for (i = 0, len = features.length; i < len; ++i) {
                  feature = features[i];
                  bounds.extend(feature.geometry.getBounds());
                  layerTemplate && htmlAux.push(this.renderTemplate(itemTemplate, feature, oContextFeature));
                  if (feature.fid) {
               ...
               if (layerTemplate) {
                  if (htmlAux.length) {
                     html = this.renderTemplate(layerTemplate, {
                        layer : layer,
         ...
         getSingleHtml : function(feature) {
            var html = '', hasTemplate = false, sTemplate = this.templates.single;
            if (sTemplate) {
               html = this.renderTemplate(sTemplate, feature, this.featureContext);
               hasTemplate = true;
            }
            return {
         ...
         showSingleFeatureById : function(featureId, origin) {
            ...
            var clearPopup = true;
            if (featureId) {
               var i, len, feature, found = false, layer = this.layer, features = layer.features;
               for (i = 0, len = features.length; i < len; i++) {
                  feature = features[i];
                  if (feature.cluster) {
                  ...
                  if (found && !feature.cluster) {
                     ...
                     if (template) {
                        popupObj.showPopup({
                           ...
                           layer : layer,
                           feature : feature
                        }, feature.geometry.getBounds().getCenterLonLat(), this.renderTemplate(template, feature, this.featureContext), true, origin);
                        clearPopup = false;
                     }
                     break;
                  }
