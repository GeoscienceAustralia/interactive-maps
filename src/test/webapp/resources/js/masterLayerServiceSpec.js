...
describe(
    ...
    function () {
        ...
        it('Should load layers from the master layer list', function () {
            var layers = [
                {
                    "slug": "Airborne_Surveys_2014",
                    "visibility": false
            ...
            masterLayerService.initMasterLists().then(function () {
                var layerList = masterLayerService.loadFromMasterLayerList(layers);
                expect(layerList[0].mapType === "WMS").toBe(true);
                expect(layerList[0].visibility === false).toBe(true);
                expect(layerList[0].url === "http://www.ga.gov.au/gisimg/services/earth_science/Geoscience_Australia_National_Geophysical_Grids/MapServer/WMSServer").toBe(true);

                expect(layerList[1].mapType === "WMS").toBe(true);
                expect(layerList[1].visibility === true).toBe(true);
                expect(layerList[1].url === "http://www.ga.gov.au/gis/services/earth_science/Geoscience_Australia_Geochronology/MapServer/WMSServer").toBe(true);
            });

            $httpBackend.flush();
        });

        it('Should load tools from the master tool list', function () {
            var tools = [
                {
                    "id": "addLayer"
                },
            ...
            masterLayerService.initMasterLists().then(function () {
                var toolList = masterLayerService.loadFromMasterToolList(tools);
                expect(toolList[0].id === "addLayer").toBe(true);
                expect(toolList[0].toolPanelUrl === "resources/partial/addLayerPanel.html").toBe(true);
                expect(toolList[0].toolToggleUrl === "resources/partial/addLayerToggle.html").toBe(true);

                expect(toolList[1].id === "clientMeasure").toBe(true);
                expect(toolList[1].toolPanelUrl === "resources/partial/measurePanel.html").toBe(true);
                expect(toolList[1].toolToggleUrl === "resources/partial/measureToggle.html").toBe(true);
            });
