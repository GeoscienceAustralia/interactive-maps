...
app.directive('geoLayerPanel', ['$timeout', '$q', '$log', function($timeout,$q,$log) {
    return {
        restrict: 'E',
        scope: {
            geoConfig: '=',
        ...
        templateUrl:'resources/js/layers/layer-panel.html',
        link: function (scope,el,attrs) {
            ...
            function updateMapBackgroundColour(layerId) {
                for (var i = 0; i < scope.geoConfig.baseMaps.length; i++) {
                    ...
                    if (baseMap.id === layerId && (baseMap.mapBGColor || scope.geoConfig.mapBGColor)) {
                        $('#' + scope.mapElementId).css('background-color', baseMap.mapBGColor || scope.geoConfig.mapBGColor);
                        break;
                    }
                }
            }
            ...
            function dropDownLayerChange(layerId, groupId, visibilityFromConfig) {
                ...
                if (visibilityFromConfig && configLayer.name) {
                    ...
                    var defaultVis = configLayer.visibility;
                    angular.extend(configLayer,groupLayer);
                    // When initializing layers for the first time, take visibility from layer config
                    // rather than groupLayer config.
                    configLayer.visibility = defaultVis;
                } else {
                    // Update map config
                    angular.extend(configLayer,groupLayer);
